import { type DataRecord, React, hooks, lodash, type ClauseValuePair, DataSourceStatus, dataSourceUtils, Immutable, type FeatureDataRecord } from 'jimu-core'
import FeatureTable from 'esri/widgets/FeatureTable'
import * as reactiveUtils from 'esri/core/reactiveUtils'
import { LayerHonorModeType, type LayersConfig, ResponsiveType, SelectionModeType } from '../../config'
...
const useTable = (props: UseTableOptions): [__esri.FeatureTable, boolean, string] => {
  const {
    dataInvalid, layerConfig, activeView, enableAttachments, enableRelatedRecords, tableContainer,
    canEditFeature, selectQueryFlag, tableShowColumns, dataActionRecords, selectionViewIds, maxCount,
    sourceVersion, isMapMode, tableSelectAsyncDs, isSelfEditing, activeTabId
  } = props
  ...
  const bindTableClickEvent = React.useCallback(() => {
    const tableWidget = tableRef.current
    const selectMode = layerConfig?.selectMode
  ...
  }, [isTableEditing, layerConfig?.selectMode, resetTableExpression, selectQueryFlag, tableSelectAsyncDs])
  ...
  }, [layerConfig?.enableSelect, isTableEditing, bindTableClickEvent, bindTableKeyDownEvent])

  const updateTableByConfig = React.useCallback((configChange?: boolean) => {
    const tableWidget = tableRef.current
    if (!layerConfig || !tableWidget) return
    const tableTemplate = constructTableTemplate(dataSource, layerConfig, tableShowColumns, configChange)
