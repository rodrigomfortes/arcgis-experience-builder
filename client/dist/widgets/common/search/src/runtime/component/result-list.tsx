...
const ResultList = (props: ResultListProps) => {
  ...
  const loadRecords = () => {
    ...
    Promise.all([serviceRecords]).then(res => {
      ...
      allResponse.forEach(dsResult => {
        ...
        let disPlayData = getDisplayRecords(records, configId, dsId, isGeocodeRecords, localDsId)
  ...
  */
  ...
  const onSelectRecord = hooks.useEventCallback((option: SelectRecordsOption) => {
    ...
    publishRecordsSelectionChangeMessage(dsId, localDsId, records)
  ...
  const publishRecordsSelectionChangeMessage = (dsId: string, localDsId: string, records: DataRecord[]) => {
    if(localDsId) {
