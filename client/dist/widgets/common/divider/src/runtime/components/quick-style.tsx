...
const QuickStyleComponent = (props: (ToolSettingPanelProps | MobileSidePanelContentOptions) & ExtraProps) => {
    ...
    return css`
      ...
      .quick-style-item {
        ...
        &.quick-style-item-selected {
          border: 2px solid ${(builderTheme as any).sys.color.primary.light};
        }
        .quick-style-item-inner {
          background-color: ${(appTheme as any).ref.palette.neutral[300]};
          cursor: pointer;
        }
      }
    `
  ...
  const quickStyleComponent = () => {
    ...
    const quickStyleComponent = []
    const QuickStyleConfig = getQuickStyleConfig(appTheme as any)
    let index = -1
