...
import { useDefaultLabel, useNeedHideItemCategories } from '../../../utils'
...
export const DataItemSearch = (props: DataItemSearchProps) => {
  ...
  const shouldBeDisplayedItemCategoriesInfo = useMemo(() => {
    return itemCategoriesInfo ? itemCategoriesInfo.filter((item) => item.enabled && !needHideItemCategories.includes(item.type)).map((item) => ({ id: item.id, type: item.type, customLabel: item.customLabel || getDefaultLabel(item), curatedFilter: item.curatedFilter })).asMutable() : undefined
  }, [getDefaultLabel, itemCategoriesInfo, needHideItemCategories])
