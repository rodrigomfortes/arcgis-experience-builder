...
import { type WebChartFixedDateBinningUnits, getSeriesType } from 'jimu-ui/advanced/chart'
 ...
 */
export const getAppropriateTimeUnit = (startTime: number, endTime: number, minPeriod = 3, maxPeriod = 50): WebChartFixedDateBinningUnits => {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    if (candidate.count >= minPeriod && candidate.count <= maxPeriod) {
      return DateTimeUnitsMap[candidate.unit] as WebChartFixedDateBinningUnits
    }
  }

  return DateTimeUnitsMap[candidates[0].unit] as WebChartFixedDateBinningUnits
}
