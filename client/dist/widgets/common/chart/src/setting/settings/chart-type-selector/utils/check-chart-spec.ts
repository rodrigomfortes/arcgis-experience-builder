...
import { getSeriesType, type WebChart, WebChartCurrentVersion, type WebChartSeriesType } from 'jimu-ui/advanced/chart'
...
/**
 * Check if the version of `WebChart` is supported.
...
const checkVersion = (WebChart: WebChart): SpecValidationStatus => {
  let valid = true
  let error = ''

  valid = WebChart.version === WebChartCurrentVersion
  if (!valid) {
    error = `Unsupported versions: ${WebChart.version}.`
  }
  return { valid, error }
}
