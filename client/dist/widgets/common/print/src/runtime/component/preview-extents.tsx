/** @jsx jsx */
import { React, jsx, css, lodash, loadArcGISJSAPIModules, loadArcGISJSAPIModule, Immutable, hooks, polished, ReactRedux, type IMState, WidgetState } from 'jimu-core'
...
const PreviewExtent = (props: Props) => {
  ...
  const previewExtentRef = useRef(false)
  const STYLE = css`
    .checkbox-con {
  ...
  const handlePreviewExtent = () => {
    ...
    previewExtentRef.current = !previewExtent
    if (!previewExtent) {
      showGraphicOnMap()
    } else {
      removeAllGraphics()
  ...
  }, [curExtent, curScale, config.previewBackgroundColor, config.previewOutLine, previewExtent])
    ...
    if (jimuMapView) {
      getReactiveUtils().then(() => {
        watchExtentChangeHandleRef.current = reactiveUtilsRef.current?.watch(() => jimuMapView?.view?.extent, () => {
          const newCurExtent = JSON.parse(JSON.stringify(jimuMapView?.view?.extent.toJSON()))
  ...
  const showGraphicOnMap = hooks.useEventCallback(() => {
    if (widgetsRuntimeInfoRef.current?.state === WidgetState.Hidden || widgetsRuntimeInfoRef.current?.state === WidgetState.Closed) {
      return
    }
    ...
    if (!previewExtentRef.current) {
      //When you press the Space key quickly, twice, may cause preview Graphics to always exist
      removeAllGraphics()
    }
  })
