import { Immutable, expressionUtils, dataSourceUtils, type Expression, type DuplicateContext, type extensionSpec, type IMAppConfig, type LinkParam, type IMExpression, dynamicStyleUtils, type IMWidgetJson, type ArcadeContentConfig, arcadeContentUtils } from 'jimu-core'
...
export default class AppConfigOperation implements extensionSpec.AppConfigOperationsExtension {
  ...
  useDataSourceWillChange(appConfig: IMAppConfig, dataSourceId: string, newDataSourceId?: string): IMAppConfig {
    ...
    if (dataSourceId || newDataSourceId) {
      ...
      const text = replaceDataSourcesForText(config.text, useDataSourceIds, prevUseDataSourceIds, this.widgetId, appConfig)
      if (text) {
    ...
    if (config.style?.enableDynamicStyle) {
      ...
      const newUseDataSources = widgetJson.useDataSources.filter(useDs => useDs.dataSourceId !== dataSourceId).concat(dataSourceUtils.getUseDataSourceByDataSourceId(newDataSourceId))
