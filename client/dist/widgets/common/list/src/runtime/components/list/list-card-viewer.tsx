...
export default class ListCardViewer extends ListCard {
  ...
  getBackgroundStyle = () => {
    const { active, appMode, isHover, config, dynamicStyleOfCard, previewDynamicStyle, builderStatus } = this.props
    const { cardConfigs } = config
    const selectable = cardConfigs[Status.Selected].selectionMode !== SelectionModeType.None
    const hoverLayoutOpen = cardConfigs[Status.Hover].enable
    const isInBuilder = window.jimuConfig.isInBuilder
    ...
    if (previewDynamicStyle && builderStatus) {
      ...
      bgStyle = initBackgroundStyle(cardConfigs[builderStatus].backgroundStyle, dynamicStyleOfCard?.[builderStatus])
      return bgStyle
    }
    if (isInBuilder && appMode === AppMode.Design) {
      bgStyle = initBackgroundStyle(cardConfigs[Status.Default].backgroundStyle, dynamicStyleOfCard?.[Status.Default])
    } else {
      bgStyle = initBackgroundStyle(cardConfigs[Status.Default].backgroundStyle, dynamicStyleOfCard?.[Status.Default])

      if (hoverLayoutOpen && isHover) {
        bgStyle = initBackgroundStyle(cardConfigs[Status.Hover].backgroundStyle, dynamicStyleOfCard?.[Status.Hover])
      }
      if (selectable && active) {
        bgStyle = initBackgroundStyle(cardConfigs[Status.Selected].backgroundStyle, dynamicStyleOfCard?.[Status.Selected])
      }
    }
    return bgStyle
  }
