...
const DataCountComponent = (props: Props) => {
  ...
  const scrollToSelectedItemsTimeoutRef = useRef(null)

  const { useDataSources, id, config, activeSort, showLoadingWhenConfirmSelectTemplate, noLayoutEntry } = props
  const { updateQueryOptions, loadNextPageWhenListSizeOrTotalCountChange, updateCreateDataSourceFailed, scrollToSelectedItems } = props
  const [query, setQuery] = useState({
    returnGeometry: true
  } as any)

  const { page, searchText, dataSource, showSelectionOnly, currentFilter, filterApplied, sortOptionName, pageSize, queryStatus, hasDsLoadedRecord } = useListRuntimeState()
  ...
  const getDsRecords = React.useCallback((showSelectionOnly, appMode) => {
    ...
    //This is a temporary solution in patch, and the next release needs to be optimized
    clearTimeout(scrollToSelectedItemsTimeoutRef.current)
    scrollToSelectedItemsTimeoutRef.current = setTimeout(() => {
      dataSourceRef.current && scrollToSelectedItems(dataSourceRef.current)
    }, 100)
  }, [listRuntimeDispatch, getLoadingStatus, scrollToSelectedItems])
  ...
  const onDsSelectionChange = hooks.useEventCallback((selection) => {
    if (dataSource) {
      getDsRecords(showSelectionOnly, appMode)
      dataSource && scrollToSelectedItems(dataSource)
    }
  })
