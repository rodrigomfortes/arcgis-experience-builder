...
const ListComponent = (props: Props) => {
  ...
  const { dataSource, records, currentCardSize, widgetRect, pageSize, isShowEditingMask } = useListRuntimeState()
  ...
  const handleItemChange = (itemRecord: DataRecord) => {
  ...
  const updateCanClickLinkWhenClickItem = (itemRecord: DataRecord) => {
    if (!dataSource || !itemRecord || !itemRecord?.getId) return
  ...
  const isEditing = (listProps: ListProps): boolean => {
  ...
  const getCardSizeWidthUnitByConfig = () => {
  ...
  const getOtherProps = hooks.useEventCallback((listProps: ListProps) => {
  ...
  const changeIsResizingCard = (isResizingCard: boolean) => {
    listRuntimeDispatch({type: 'SET_IS_RESIZING_CARD', value: isResizingCard})
  }

  const getItemsByRecords = hooks.useEventCallback((records, listProps, hoverIndex: number, hideCardTool: boolean) => {
  ...
  return (<Fragment>
    <div className='widget-list-link-con position-relative' data-testid='listContainer' onKeyUp={handleListKeyUp}>
      <LinkContainer
        widgetId={id}
        appMode={appMode}
