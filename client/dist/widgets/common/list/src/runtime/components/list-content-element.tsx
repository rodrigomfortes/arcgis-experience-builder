...
import { getListContentLeftPadding, getItemColumnCount, initNewCardSize, getListVisibleStartIndex,  getListVisibleEndIndex} from '../utils/list-element-util'
...
export class Widget extends React.PureComponent<ListProps & Props, ListStates> {
  ...
  handleScrollDown = e => {
    ...
    if (e) {
      ...
      listVisibleEndIndex = getListVisibleEndIndex({lastScrollOffset, config, currentCardSize, columnCount, listItemContentSize})
      this.isScrollToNextStep = true
    } else {
      //Execute after Record load, jump according to the first visible line
      listVisibleEndIndex = getListVisibleStartIndex({lastScrollOffset, config, currentCardSize, columnCount})
    }
    const scrollStep = this.getScrollStep()
