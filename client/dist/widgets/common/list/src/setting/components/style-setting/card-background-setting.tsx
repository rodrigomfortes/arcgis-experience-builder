...
const CardBackgroundSetting = (props: Props) => {
  ...
  const borderStyle = config.cardConfigs?.[showCardSetting]?.backgroundStyle?.border?.asMutable({ deep: true })
  return (
    <Fragment>
      <SettingSection className={`clear-padding ${isClearBorder && 'clear-border clear-padding-bottom'}`}>
        <SettingRow label={nls('background')} flow='wrap' role='group' aria-label={nls('background')}>
          <BackgroundSetting
            ...
            onChange={value => { onBackgroundStyleChange(showCardSetting, 'background', value) }}
          />
        </SettingRow>
      </SettingSection>
      <SettingSection className={`clear-padding ${isClearBorder && 'clear-border clear-padding-bottom'}`}>
