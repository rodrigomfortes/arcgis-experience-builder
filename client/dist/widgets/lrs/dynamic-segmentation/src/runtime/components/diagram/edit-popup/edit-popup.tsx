...
export function EditPopup (props: EditPopupProps) {
  const { widgetId, track, trackRecord, trackFieldInfos, lrsLayers, subtypeLayers, networkInfo, showEventStatistics, featureLayer, onApply, onClose } = props
  ...
  const handleApply = React.useCallback((record: TrackRecord) => {
    ...
    query.outFields = ['*']
    query.where = featureLayer.objectIdField + ` = ${record.objectId}`
