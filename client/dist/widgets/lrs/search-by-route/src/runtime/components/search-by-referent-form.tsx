...
import { Intellisense, getSuggestions, type Suggestion, GetUnits, isDefined } from 'widgets/shared-code/lrs'
  ...
  const validateReferentTextInput = (value: string | number): Promise<ValidityResult> => {
    ...
    return new Promise(resolve => {
      setTimeout(() => {
        ...
        if (isDefined(referentItem?.fromDateFieldName)) {
          featureQuery.outFields.push(referentItem?.fromDateFieldName)
        }
