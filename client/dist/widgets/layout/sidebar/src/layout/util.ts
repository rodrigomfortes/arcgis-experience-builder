import { appActions, getAppStore, type IMAppConfig, LayoutItemType } from 'jimu-core'
import { CollapseSides } from '../config'

export function syncWidgetHiddenState (widgetId: string, showCollapse: boolean) {
  const appState = getAppStore().getState()
  const widgetJson = appState.appConfig.widgets[widgetId]
  const firstLayouts = widgetJson.layouts.FIRST
  const secondLayouts = widgetJson.layouts.SECOND
  const browserSizeMode = appState.browserSizeMode
  const mainSizemode = appState.appConfig.mainSizeMode
  let layoutId: string

  if (widgetJson.config.collapseSide === CollapseSides.First) {
    layoutId = firstLayouts[browserSizeMode] ?? firstLayouts[mainSizemode]
  } else {
    layoutId = secondLayouts[browserSizeMode] ?? secondLayouts[mainSizemode]
  }

  if (showCollapse) {
    getAppStore().dispatch(appActions.showWidgetsInLayout(layoutId))
    getAppStore().dispatch(appActions.widgetStatePropChange(widgetId, 'hiddenLayoutIds', []))
  } else {
    getAppStore().dispatch(appActions.hideWidgetsInLayout(layoutId))
    getAppStore().dispatch(appActions.widgetStatePropChange(widgetId, 'hiddenLayoutIds', [layoutId]))
  }
}
