...
export interface TreeRootProps {
  ...
  root: NodeRepositoryItem
  rootItems: IRepositoryItem[]
  selectedRootItem: NodeRepositoryItem | null
  hidden?: boolean
}

const TreeRoot = (props: TreeRootProps): JSX.Element => {
  const { i18nMessage, theme, root, rootItems, selectedRootItem, hidden } = props
  ...
  const renderRoot = (): JSX.Element => {
    ...
    return (
      <>
        <div css={treeNodeStyle(theme)} onClick={() => { onToggleExpanded() }}>
          {rootExpanded
            ? (
                <div css={treeNodeIcon(theme)}><DownFilled size="s" /></div>
              )
            : (
                <div css={treeNodeIcon(theme)}><RightFilled size="s" /></div>
              )}
          <b>{root.label}</b>
        </div>
      </>
  ...
  const renderRootItems = (): JSX.Element[] => {
    if (!(rootItems?.length > 0)) {
      return [
        <div key="no-projects" hidden={!rootExpanded} css={noDocumentsStyle(theme)}>{i18nMessage('noProjects')}</div>
      ]
    }
    return rootItems
  ...
  return (
    <div hidden={hidden}>
      <div css={treeContainer(theme)}>
        {renderRoot()}
        <div css={selectedRootItem === null ? rootDisplayStyle(theme) : rootItemsDisplayStyle(theme)}>
          {renderRootItems()}
        </div>
      </div>
    </div>
