...
import { treeChildrenStyle, noDocumentsStyle, treeNodeStyle, treeNodeIcon, treeNodeLoadingStyle } from '../../styles'
  ...
  const renderDocumentChildren = (): JSX.Element[] => {
    if (documentChildren.length === 0) {
      if (nodeChildren.length === 0) {
        ...
        return [
          <div key="no-documents" css={noDocumentsStyle(theme)}>{i18nMessage('noDocuments')}</div>
        ]
      } else {
        return []
      }
