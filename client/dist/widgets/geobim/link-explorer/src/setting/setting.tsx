/** @jsx jsx */
import { jsx, React, getAppStore } from 'jimu-core'
...
interface ModelViewerWidgets {
  [widgetKey: string]: string // (widgetKey: widgetLabel)
}

const LinkExplorerWidgetSetting = (props: AllWidgetSettingProps<IMConfig>): JSX.Element => {
  ...
  const removeOutdatedModelViewerWidgetId = (modelViewerWidgets: ModelViewerWidgets): void => {
    if (modelViewerWidgetId !== '' && modelViewerWidgets[modelViewerWidgetId] == null) {
      onModelViewerWidgetSelected('')
    }
  }

  const getModelViewerWidgets = (): ModelViewerWidgets => {
    const appWidgets = getAppStore().getState().appStateInBuilder.appConfig.widgets
    const modelViewerWidgets: ModelViewerWidgets = {}
    Object.keys(appWidgets).forEach((key) => {
      if (appWidgets[key].manifest.name === GeoBIMWidgetNames.DOCUMENT_VIEWER) {
      ...
      <SettingSection title={i18nMessage('settingsLabel')}>
        ...
        <SettingRow>
          <FeatureServiceErrors
            ...
            infoMessageText={i18nMessage('geobim_bimCloudConnectionSettingsTip')}
          />
        </SettingRow>
