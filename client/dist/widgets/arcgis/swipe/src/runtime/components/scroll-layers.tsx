...
export function ScrollLayers (props: ScrollLayersProps) {
  ...
  const firstMapSizeWatchHandleRef = React.useRef<__esri.WatchHandle>()
  const secondMapSizeWatchHandleRef = React.useRef<__esri.WatchHandle>()
        ...
        if (inactiveMapLayerViewsPromiseArray) {
          Promise.all(inactiveMapLayerViewsPromiseArray).then(jimuLayerViews => {
            if (swipeDirection === 'vertical') {
              secondMapSizeWatchHandleRef.current = inactiveMapView?.view?.watch('height', () => { updateSizeSecond() })
            } else {
              secondMapSizeWatchHandleRef.current = inactiveMapView?.view?.watch('width', () => { updateSizeSecond() })
            }
            updateSizeSecond()
