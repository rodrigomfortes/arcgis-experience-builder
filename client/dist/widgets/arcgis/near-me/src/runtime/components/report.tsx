...
export default class Report extends React.PureComponent<Props, State> {
  ...
  handleAttribution = () => {
    if (this.props.mapView.view != null) {
      ...
      const attributionContainer = attributionTool?.[0]?.instance?.container
      const attribution = attributionTool?.[0]?.instance?.container?.className?.includes('esri-attribution')
  ...
  handleCompass = () => {
    if (this.props.mapView.view != null) {
      ...
      const compassContainer = compassTool?.[0]?.instance?.container
      const compass = compassTool?.[0]?.instance?.container?.className?.includes('esri-compass')
      if (compass && compassContainer != null && typeof compassContainer !== 'string') {
        const compassNode: any = compassContainer.cloneNode(true)
        compassNode.children[0].children[0].children[0].innerHTML = '<svg aria-hidden="true" fill="currentColor" height="25" width="25" xmlns="http://www.w3.org/2000/svg" class=" svg " viewBox="0 0 24 24"><!--?lit$499456230$--><!----><path d="M12 22.981l4.12-11.49L12 1.149 7.88 11.49zM9.125 12h5.75L12 20.019z"></path><!----></svg>'
        return compassNode
      }
    }
  }
   ...
   * @returns scale bar conatiner
   */
  handleScaleBar = () => {
    if (this.props.mapView.view != null) {
      const scaleBarTool = this.props.mapView.jimuMapTools.filter((tools) => { return tools.name === 'ScaleBar' })
      const scaleBarContainer = scaleBarTool?.[0]?.instance?.container
      const scaleBar = scaleBarTool?.[0]?.instance?.container?.className?.includes('esri-scale-bar')
      if (scaleBar && scaleBarContainer != null && typeof scaleBarContainer !== 'string') {
        return scaleBarContainer.cloneNode(true)
      }
    }
  }
