...
const Widget = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (headElement) {
      if (!headElement.getElementsByClassName(linkClassName)?.length) {
        ...
        linkElement.href = `${getAssetsPathByFolderName('arcgis-ba-data-browser')}main.css`
      ...
      if (!headElement.getElementsByClassName(scriptClassName)?.length) {
        ...
        scriptElement.src = `${getAssetsPathByFolderName('arcgis-ba-data-browser')}index.js`
  ...
  const analysisCoreRef = useRef<typeof AnalysisCoreType>()
  useEffect(() => {
