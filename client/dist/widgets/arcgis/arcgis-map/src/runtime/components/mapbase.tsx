...
import {
  ...
  getDefaultHighlightOptions, parseMarkerGroupUrlParam
} from '../utils'
...
export default class MapBase extends React.PureComponent<Props, State> {
  ...
  async cacheLoadModulesAndCreateMapbaseView (dataSourceInfo: MapbaseDataSourceInfo): Promise<void> {
    ...
    try {
      ...
      this.createMapbaseView(mapCreatingInfo)
    } catch (e) {
  ...
  createMapbaseView (mapCreatingInfo: MapCreatingInfo): void {
  ...
  // This method try to recreate mapbaseview only for that case that layers that previously failed to load are now accessible.
  // This method doesn't handle map ds recreated case, onDataSourceCreated will handle the case.
  ...
  onCreateDataSourceFailed = (err): void => {
    ...
    // Update mapLoadStatus state will trigger this.tryCreateMapViewWhenDidUpdate() method to create this.view.
    // So we need to put the following code into a timer after updating mapLoadStatus.
    setTimeout(() => {
      // If webmap/webscene is private and user clicks the cancel button of login dialog, then view.map.loadStatus is changed to 'failed', but the errCallback of view.when() doesn't trigger.
      // To check map.loadStatus, we can use webmap.when(), or use onCreateDataSourceFailed callback.
      // Here we use onCreateDataSourceFailed because it can handle some common code logic of ds create failed and webmap failed, such as changing mapLoadStatus.
      if (this.view) {
