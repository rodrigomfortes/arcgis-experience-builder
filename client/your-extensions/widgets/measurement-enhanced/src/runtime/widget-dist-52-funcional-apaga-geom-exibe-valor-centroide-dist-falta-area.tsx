...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  // Gera ou retorna o id único do gráfico
  ...
  // --- START: Cria camada só uma vez e mantém durante todo ciclo de vida do Widget ---

  // Flag para evitar recriar camada
    ...
    loadArcGISJSAPIModules([
        ...
        if (event.state === "active" || event.state === "complete") {
          ...
          if (idx >= 0) {
            ...
            if (event.state === "active") {
              ...
              console.log("[LOG] Live measurement during update:", liveValue);
              setLiveMeasurement(liveValue);
      ...
      clickHandlerRef.current = jimuMapView.view.on("click", async (evt) => {
        ...
        if (!graphicHit && candidates.length > 0) {
          // Caso não encontre no graphicsRef, vamos forçar a seleção pelo primeiro candidato
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Usa a camada para encontrar o gráfico diretamente
      ...
      // Atualiza o array local e estado
