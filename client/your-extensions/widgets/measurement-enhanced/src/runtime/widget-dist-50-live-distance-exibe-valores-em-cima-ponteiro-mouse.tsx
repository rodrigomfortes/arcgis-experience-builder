...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  const [liveMeasurement, setLiveMeasurement] = useState<string | null>(null); // Medição ao vivo
  ...
  // Atualiza o texto da medição ao vivo no mapa sobre a geometria atual
  const updateLiveTextGraphic = (geometry: __esri.Geometry, text: string) => {
    ...
    // Calcula o ponto para o label: centro para polígono, último ponto para linha
    ...
    if (!liveTextGraphicRef.current) {
      ...
      // Não chamar refresh(), atualização automática
    }
  };
  ...
  // Limpa todos gráficos e estados
  const clearAll = () => {
  ...
  useEffect(() => {
    ...
    console.log("[DEBUG] jimuMapView is ready");
  }, [jimuMapView]);
    ...
    loadArcGISJSAPIModules([
      ...
      createHandlerRef.current = sketchRef.current.on("create", (event) => {
        if (
          ...
          console.log(`[DEBUG] Live measurement during create: ${liveValue}`);
        }
        ...
        if (event.state === "active" || event.state === "complete") {
          ...
          if (idx >= 0) {
            ...
            if (event.state === "active") {
              ...
              console.log(`[DEBUG] Live measurement during update: ${liveValue}`);
            } else {
              setLiveMeasurement(null);
              if (liveTextGraphicRef.current) {
                graphicsLayerRef.current?.remove(liveTextGraphicRef.current);
