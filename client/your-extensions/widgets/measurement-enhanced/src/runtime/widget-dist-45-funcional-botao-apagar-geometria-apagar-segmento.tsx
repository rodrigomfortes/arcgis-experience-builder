...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  const [deleteMode, setDeleteMode] = useState<'geometry' | 'vertex'>('geometry') // modo padrão geometria
  ...
  const drawAllLabels = () => {
    ...
    graphicsRef.current.forEach(graphic => {
      ...
      // Pontos vértices dourados (#AC8D43)
      path.forEach(coord => {
  ...
  // Atualiza símbolos para mostrar destaque da seleção
  ...
  // Setup clique na linha para selecionar e destacar
  ...
  // Trata tecla Delete para apagar segmento ou geometria toda conforme modo
  useEffect(() => {
    if (!jimuMapView) return

    const handleKeyDown = (event: KeyboardEvent) => {
      if ((event.key !== 'Delete' && event.key !== 'Backspace') || !selectedGraphicId) return
      ...
      if (deleteMode === 'vertex') {
        ...
        // Remove vértices da geometria, só se tiver mais de 2 pontos (no mínimo uma linha)
        ...
        // Modo geometria: apaga toda a geometria
  ...
  }, [selectedGraphicId, deleteMode, jimuMapView])
  ...
  // Botão alternar modo de exclusão estilizado como botão azul
  const DeleteModeButton = () => (
    <button
      type="button"
      onClick={() => setDeleteMode(deleteMode === 'vertex' ? 'geometry' : 'vertex')}
      title="Alternar modo de exclusão"
      aria-label="Alternar modo de exclusão"
      style={{
    ...
    >
      {deleteMode === 'vertex' ? 'Apagar Segmento' : 'Apagar Geometria'}
    </button>
  )
  ...
  return (
    <div className="widget-starter jimu-widget d-flex flex-column" style={{ height: '100%' }}>
      ...
      {/* Botão alternar modo de exclusão - aparece só se tiver seleção */}
      {selectedGraphicId && (
        <div
          style={{
            display: 'flex',
