...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  const createVertexPointsForGraphic = (graphic: __esri.Graphic) => {
    if (!graphicsLayerRef.current || !graphicModuleRef.current) return;
    ...
    if (graphic.geometry.type === "polyline") {
      points = (graphic.geometry as __esri.Polyline).paths[0];
    } else if (graphic.geometry.type === "polygon") {
      points = (graphic.geometry as __esri.Polygon).rings[0];
    } else {
      return;
    }
    ...
    vertices.forEach((v) => graphicsLayerRef.current!.add(v));
