...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  // Controle da tecla Delete para remover gráfico selecionado
  useEffect(() => {
    if (!jimuMapView || !sketchRef.current) return

    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Delete" || event.key === "Backspace") {
        const selectedGraphics = sketchRef.current?.updateGraphics?.filter(g => g.selected) || []
        if (selectedGraphics.length === 0) return

        selectedGraphics.forEach(graphic => {
          graphicsLayerRef.current?.remove(graphic)
          graphicsRef.current = graphicsRef.current.filter(g => g !== graphic)
          removeMeasurementDetails(graphic)
        })

        drawAllLabels()

        // Atualiza lista (força re-render)
    ...
    loadArcGISJSAPIModules([
      ...
      createHandlerRef.current = sketchRef.current.on('create', (event) => {
        if (event.state === 'complete' && event.graphic.geometry.type === 'polyline') {
          ...
          console.log('Medição criada:', event.graphic)
        }
      })
      ...
      updateHandlerRef.current = sketchRef.current.on('update', (event) => {
        if (event.state === 'active' || event.state === 'complete') {
          ...
          if (idx >= 0) {
            ...
            console.log('Medição atualizada:', editedGraphic)
          }
        }
      })
      ...
      deleteHandlerRef.current = sketchRef.current.on('delete', (event) => {
        ...
        event.graphics.forEach(deletedGraphic => {
          ...
          console.log('Medição removida:', deletedGraphic)
        })
        drawAllLabels()
      })
