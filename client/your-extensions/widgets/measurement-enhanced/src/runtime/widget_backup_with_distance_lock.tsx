...
const Widget = (props: AllWidgetProps<any>) => {
  ...
  const removeMeasurementDetails = useCallback((graphic: __esri.Graphic) => {
   ...
   const clearAll = () => {
     ...
     // Limpar travamento de distância
     setIsDistanceLocked(false)
     setLockedDistance(null)
     removeDistanceLockIndicator()
  ...
  const removeAreaLabel = (graphicId: string) => {
    ...
    if (oldAreaLabel && graphicsLayerRef.current) {
      graphicsLayerRef.current.remove(oldAreaLabel)
    }
  ...
  const createAreaLabelForGraphic = useCallback((graphic: __esri.Graphic) => {
    if (!graphicsLayerRef.current || !graphicModuleRef.current || graphic.geometry.type !== "polygon") return
    ...
    graphicsLayerRef.current.add(areaLabel)
  ...
  useEffect(() => {
    ...
    loadArcGISJSAPIModules([
      ...
      graphicsLayerRef.current = gl
       ...
       clickHandlerRef.current = jimuMapView.view.on("click", async (evt) => {
         ...
         // Se travamento de distância estiver ativo, interceptar o clique
         if (isDistanceLocked && sketchRef.current?.state === "active" && activeButton === "measureDistance") {
           const mapPoint = jimuMapView.view.toMap(evt)
           const lockedPoint = calculateLockedPosition(mapPoint)
           
           if (lockedPoint && lastValidPoint) {
             console.log("[LOG] Clique com travamento de distância ativo")
             console.log("[LOG] Ponto original:", mapPoint)
             console.log("[LOG] Ponto travado:", lockedPoint)
             
             // Atualizar o último ponto válido
             setLastValidPoint(lockedPoint)
             
             // Criar indicador visual do raio
             createDistanceLockIndicator(lockedPoint)
             
             // O SketchViewModel continuará normalmente, mas o usuário verá o raio
             return
           }
         }
    ...
    return () => {
      ...
      mouseMoveHandlerRef.current?.remove()
    }
   }, [jimuMapView, createAreaLabelForGraphic, createSegmentLabelsForGraphic, createVertexPointsForGraphic, formatLastSegmentDistance, formatLiveMeasurement, removeMeasurementDetails, updateMeasurementDetails, activeButton, checkNearStartPoint, isNearStartPoint, startPoint, configureSnappingSources, createRightAngleIndicator, detectRightAngle, isRightAngle, removeRightAngleIndicator, optimizeSnappingPerformance, isDistanceLocked, calculateLockedPosition, createDistanceLockIndicator, lastValidPoint])

  // Delete com tecla Delete ou Backspace, ESC para cancelar desenho e D para travamento de distância
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      // Tecla D para travamento de distância
      if (event.key === "d" || event.key === "D") {
        handleDistanceLockToggle()
      ...
      if (event.key === "Escape") {
        if (sketchRef.current?.state === "active") {
          ...
          // Força limpeza de todos os gráficos de texto na camada
          if (graphicsLayerRef.current) {
            const allGraphics = Array.from(graphicsLayerRef.current.graphics)
            ...
            if (textGraphics.length > 0) {
              ...
              graphicsLayerRef.current.removeMany(textGraphics)
            }
          }
          
          // Limpa travamento de distância
          setIsDistanceLocked(false)
          setLockedDistance(null)
          removeDistanceLockIndicator()
  ...
  }, [jimuMapView, liveMeasurement, handleDistanceLockToggle, removeDistanceLockIndicator, lockedDistance])
  ...
  return (
    <div className="widget-starter jimu-widget d-flex flex-column" style={{ height: "100%" }}>
      ...
      {/* Informação sobre snapping universal */}
      {(activeButton === "measureDistance" || activeButton === "measureArea") && (
        ...
        >
          🔗 Snapping ativo: O cursor irá "travar" em vértices de qualquer camada automaticamente
        </div>
      )}

      {/* Informação sobre travamento de distância */}
      {activeButton === "measureDistance" && (
        <div
          style={{
            padding: "6px 12px",
            backgroundColor: isDistanceLocked ? "rgba(40, 167, 69, 0.1)" : "rgba(255, 193, 7, 0.1)",
            border: isDistanceLocked ? "1px solid rgba(40, 167, 69, 0.3)" : "1px solid rgba(255, 193, 7, 0.3)",
            borderRadius: "4px",
            margin: "4px 0",
            fontSize: "12px",
            color: isDistanceLocked ? "#155724" : "#856404",
            textAlign: "center"
          }}
        >
          {isDistanceLocked ? (
            <>🔒 Travamento ativo: {lockedDistance}m - Pressione 'D' para desativar</>
          ) : (
            <>📏 Pressione 'D' para travar distância durante medição</>
          )}
        </div>
      )}

      {/* Modal de entrada de distância */}
      {showDistanceModal && (
        <div
          style={{
            position: "fixed",
        ...
        >
          ...
          >
            <h3 style={{ margin: "0 0 15px 0", color: "#333" }}>
              Definir Distância de Travamento
            </h3>
            <p style={{ margin: "0 0 15px 0", color: "#666", fontSize: "14px" }}>
              Digite a distância em metros que será mantida fixa durante a medição:
            </p>
            <input
              type="number"
              value={distanceInput}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  handleDistanceInputCancel()
                }
              }}
            />
            <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
              <button
                onClick={handleDistanceInputCancel}
                style={{
                  padding: "8px 16px",
                  border: "1px solid #ccc",
                  backgroundColor: "white",
              ...
              <button
                onClick={handleDistanceInputSubmit}
                style={{
                  padding: "8px 16px",
                  border: "none",
